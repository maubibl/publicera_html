      ...
      <div class="row">
        ...
        <xsl:for-each select="journal-meta">
          ...
          <div class="cell">
            ...
            <xsl:apply-templates select="journal-title-group" mode="metadata"/> 
...
<!--  Includes mode "metadata" for front matter, along with 
  ...
  <xsl:template name="volume-info"> 
    ...
    <xsl:if test="normalize-space(volume) or volume-id or volume-series">
      <xsl:choose>
        <xsl:when test="not(volume-id[2]) or not(volume) or volume=''">
...
<xsl:if test="not(following-sibling::*[1][self::title])">
  ...
  <xsl:variable name="auto-label-ref" select="false()"/>
...
<!-- ============================================================= -->
  ...
  <xsl:template name="metadata-labeled-entry">
    ...
    <xsl:param name="contents">
     ...
     <xsl:if test="not(@pub-id-type='publisher-id')"> <!-- ny if -->
      <xsl:call-template name="metadata-entry">
        <xsl:with-param name="contents">
          <xsl:if test="normalize-space(string($label))">
            <span class="generated">
